...
CatFramelessWidget::CatFramelessWidget(QWidget *parent)
    : QWidget(parent)
{
    padding = 8;
    moveEnable = true;
...
void CatFramelessWidget::showEvent(QShowEvent *event)
{
...
void CatFramelessWidget::doResizeEvent(QEvent *event)
{
    // 非win系统的无边框拉伸,win系统上已经采用了nativeEvent来处理拉伸
    // 为何不统一用计算的方式因为在win上用这个方式往左拉伸会发生抖动
#ifndef Q_OS_WIN
    if(event->type() == QEvent::Resize)
    {
        ...
        // 右侧描点区域
        pressedRect[1] = QRect(width - padding, padding, padding, height - padding);
        // 上侧描点区域
...
bool CatFramelessWidget::eventFilter(QObject *watched, QEvent *event)
{
    if(watched == this)
    {
    ...
    bool CatFramelessWidget::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
#else
    bool CatFramelessWidget::nativeEvent(const QByteArray &eventType, void *message, long *result)
#endif
{
...
void CatFramelessWidget::setPadding(int padding)
{
    this->padding = padding;
}

void CatFramelessWidget::setMoveEnable(bool moveEnable)
{
    this->moveEnable = moveEnable;
}

void CatFramelessWidget::setResizeEnable(bool resizeEnable)
{
    this->resizeEnable = resizeEnable;
}

void CatFramelessWidget::setTitleBar(QWidget *titleBar)
{
