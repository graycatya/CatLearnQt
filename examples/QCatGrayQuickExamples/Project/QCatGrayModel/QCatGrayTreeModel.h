...
class QCatGrayTreeModel : public QAbstractListModel
{
    Q_OBJECT
...
public:
    ...
    QCatGrayTreeModel(const QList<QJsonObject>& nodeList, QObject* parent = nullptr);
    ~QCatGrayTreeModel() {}

    const QList<QJsonObject>& nodeList() const
    {
        return m_nodeList;
    }

    // 设置节点链表
    void setNodeList(const QList<QJsonObject>& nodeList);
